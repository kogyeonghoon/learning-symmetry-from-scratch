...
for epoch in range(epochs):
    for batch_idx,(u,) in tqdm(enumerate(train_loader)):
        ...
        xtu_transformed = xtu_transformed.view(n_intervals * batch_size,patch_size,patch_size,3,n_delta).permute(0,4,1,2,3).flatten(0,1) # to shape (n_intervals * batch_size * n_delta, patch_size , patch_size, n_delta)
        
        partials = compute_ndiff(xtu_transformed,nt,nx,dt,dx,u_scaler)
        pde_value = compute_residual(pde,partials)
        pde_value = torch.clamp(pde_value,min=th)
        
        # pde loss
       
        pde_value = pde_value.view(n_intervals * batch_size, n_delta, -1)
        loss_pde_each = torch.log(pde_value).mean(dim=(0,2))
